...
- (void) saveValue:(CDVInvokedUrlCommand*)command {
    NSMutableDictionary *args = [command.arguments objectAtIndex:0];
    NSString *metric = [args objectForKey:@"metric"];
    ...
    HKUnit *preferredUnit = [HKUnit unitFromString:unit]
    ...
    HKQuantityType *quantityType = [self getHKQuantityType:metric];
    
    if (quantityType == nil) {
        CDVPluginResult* result = [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsString:@"invalid metric was passed"];
        [self.commandDelegate sendPluginResult:result callbackId:command.callbackId];
        return;
    }
    
    NSSet *requestTypes = [NSSet setWithObjects: quantityType, nil];
    [self.healthStore requestAuthorizationToShareTypes:requestTypes readTypes:requestTypes completion:^(BOOL success, NSError *error) {
        if (success) {
            HKQuantity *quantity = [HKQuantity quantityWithUnit:preferredUnit doubleValue:[amount doubleValue]];
            HKQuantitySample *sample = [HKQuantitySample quantitySampleWithType:quantityType quantity:quantity startDate:date endDate:date];
            [self.healthStore saveObject:sample withCompletion:^(BOOL success, NSError* errorInner) {
                if (success) {
                    dispatch_sync(dispatch_get_main_queue(), ^{
                        CDVPluginResult* result = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK];
                        [self.commandDelegate sendPluginResult:result callbackId:command.callbackId];
...
- (void) readValue:(CDVInvokedUrlCommand*)command {
    ...
    [self.healthStore requestAuthorizationToShareTypes:requestTypes readTypes:requestTypes completion:^(BOOL success, NSError *error) {
        if (success) {
            ...
            [self.healthStore aapl_mostRecentQuantitySampleOfType:quantityType predicate:nil completion:^(HKQuantity *mostRecentQuantity, NSDate *mostRecentDate, NSError *errorInner) {
...
- (HKQuantityType *) getHKQuantityType:(NSString *) metric {
    if ([metric isEqualToString:@"heartrate"]) {
    ...
    } else if ([metric isEqualToString:@"calorieIn"]){
        return  [HKQuantityType quantityTypeForIdentifier:HKQuantityTypeIdentifierDietaryEnergyConsumed];
    } else if ([metric isEqualToString:@"calorieOut"]){
